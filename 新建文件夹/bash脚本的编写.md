...
这个符号组合告诉系统 该后面的参数来执行该文件.。如：`#! /bin/sh`  
...
只读变量 readonly , 只读的将不会改变。

删除变量 unset ,  unset variable_name,不能删只读变量

环境变量 所有的程序，包括shell启动的程序，都能访问环境变量。



输出到文件`echo "It is a test" > myfile`

显示命令执行结果 `echo 'date'`  这里用的是反引号。 out: Thu Jul 24 10:08:46 CST 2014
